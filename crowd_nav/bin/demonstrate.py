...
def main():
    ...
    parser.add_argument('--human_type', type=str, default='normal', choices=['aggressive', 'normal', 'cautious'])
    ...
    logging.info(' =========== Demonstrate %s ============ ', 'orca')
    ...
    if args.human_type == 'aggressive' :
        ...
        env_mods = {'safety_space' : 0.5, 'time_horizon' : 10}
    ...
    if args.name is not None :
        demonstration_file = os.path.join(args.memory_dir, '{}.pt'.format(args.name))
    else :
        if robot.visible:
    ...
    expert_policy.multiagent_training = True
    ...
    explorer = ExplorerDs(env, robot, device, args.num_frames, memory, gamma=gamma)
    ...
    human_obsv = explorer.run_k_episodes(il_episodes, 'train', update_memory=True, imitation_learning=True, noise_explore=noise_explore, progressbar=True)
