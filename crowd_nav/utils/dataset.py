...
class ImitDataset(Dataset):
    ...
    def __init__(self, data, action_space, device, vmin=0.0, horizon=3, sample=1):

        len_data = len(data)

        max_index = int(len(data[0])*sample)
        robot_data = data[0][:max_index]
        human_data = data[1][:max_index]
        action_data = data[2][:max_index]
        value_data = data[3][:max_index]
    ...
    @staticmethod
    def from_mult(data, device='cpu', vmin=0.0, horizon=3) :
        ...
        return ImitDataset(data, None, device, vmin=vmin , horizon=horizon)

class ImitDatasetTraj(Dataset):

    def __init__(self, data, action_space, device, vmin=0.0, horizon=3, sample=1, contrast=False):
        ...
        # contrastive seeds, i.e., true positions of the robot (pos) and human neighbors (neg)
        pos_seq = torch.zeros(robot_data.size(0), horizon, 2)
        neg_seq = torch.zeros(human_data.size(0), horizon, human_data.size(2), 2)
        # remove samples at the end of episodes
        vx = robot_data[1:,2]
        dx = robot_data[1:,0] - robot_data[:-1,0]
        diff = dx - vx * 0.25
        ...
        for t in range(horizon):
            dt = t + 1
            pos_seq[:-dt, t] = robot_data[dt:, :2]
            neg_seq[:-dt, t] = human_data[dt:, -1, :, :2]
            for i in range(dt):
                pos_seq[idx_done-i, t] *= 0.0
                if not self.contrast : 
                    neg_seq[idx_done-i, t] = human_data[idx_done, -1, :, :2]
                else :
                    neg_seq[idx_done-i, t] *= 0.0
        hor_pred = neg_seq.clone()
    ...
    def from_mult(data, device='cpu', vmin=0.0, horizon=3) :
        if isinstance(data, str) :
            data = torch.load(data)
        robot_data = torch.stack([sample[0][0] for sample in data])
        human_data = torch.stack([sample[0][1] for sample in data])
        actions = torch.stack([sample[1] for sample in data])
        values = torch.stack([sample[2] for sample in data])
        data = (robot_data, human_data, actions, values)
        return ImitDatasetTraj(data, None, device, vmin=vmin , horizon=horizon)

class ImitDatasetTrajMult(Dataset):

    def __init__(self, data, action_space, device, vmin=0.0, horizon=3, sample=1, contrast=False):

        self.n_enviorements = len(data)
        ...
        for env_data in data :
            #max_index = int(len(data[0])*sample)
            robot_data = env_data[0]
            human_data = env_data[1]
            action_data = env_data[2]
            value_data = env_data[3]
            ...
            self.robot_state.append(robot_data[mask].to(device))
            self.human_state.append(human_data[mask].to(device))
            self.action_target.append(action_data[mask].to(device))
            self.pos_state.append(pos_seq[mask].to(device))
            self.neg_state.append(neg_seq[mask].to(device))

        self.len_data = min(len(rs) for rs in self.robot_state)
        
        self.robot_state = [s[:self.len_data] for s in self.robot_state]
        self.human_state = [s[:self.len_data] for s in self.human_state]
        self.action_target = [s[:self.len_data] for s in self.action_target]
        self.pos_state = [s[:self.len_data] for s in self.pos_state]
        self.neg_state = [s[:self.len_data] for s in self.neg_state]

        self.robot_state = torch.stack(self.robot_state, dim=1)
        self.human_state = torch.stack(self.human_state, dim=1)
        self.action_target = torch.stack(self.action_target, dim=1)
        self.pos_state = torch.stack(self.pos_state, dim=1)
        self.neg_state = torch.stack(self.neg_state, dim=1)
...
def split_dataset(dataset, batch_size, percent_label=1.0, validation_split=0.3, is_random=False):
    ...
    logging.info("+: %d - %d", train_indices[0].item(), train_indices[-1].item())
