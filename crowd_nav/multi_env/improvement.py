...
def main() :
    ...
    for i, env_dir in enumerate(environement_dirs) :
        ...
        print('Environement {} : init reward : {:.2f}, init collision {:.2f}'.format(i, results['reward'], results['collision']))
        ...
        if args.compute_init_resutls and args.mult:
            res = test_model(m_p=None, model_type='sail_traj_simple', visible=False, n_episodes=250,\
                        env_type='orca', traj_path=init_traj_path, env_mods=mods, model=policy.model, t_fex=t_fex, pred_head=pred_head, notebook=False)
            results = res
        data['results'] = results
        torch.save(data, os.path.join(env_dir, 'data.pth'))
        suffix = '  '
